...
function Home() {
    ...
    const userCrafts = useSelector(state => state.products.userCrafts)
    ...
    const getAllCrafts = () => {
        return axios.get("http://localhost:8000/api/allcrafts/")
            .then((response) => {
                dispatch(prodActions.fetchall(response.data));
            })
            .catch((error) => {
    ...
    //     // Make a POST request using the interceptAPI instance
    //     interceptAPI.post('/usercrafts', requestData)
    //         .then(response => {
    ...
    // //       return navigate('/login');
    // //     } else {
    // //         const data = {'user': user, 'token': mytoken}
    ...
    //     const authenticateUser = () => {
    //       // Perform authentication logic here (e.g., check session, token validity, etc.)
    //       // For demonstration purposes, setting a timeout to simulate authentication
    //       setTimeout(() => {
    //         setIsLoggedIn(true);
    //         setIsLoading(false);
    ...
    //         <div>
    //           {isLoggedIn ? (
    //             <p>Welcome, User!</p> // Render authenticated content
    //           ) : (
    //             <button>Login</button> // Render login button if user is not authenticated
    //           )}
    //         </div>
    //       )}
    //     </div>
